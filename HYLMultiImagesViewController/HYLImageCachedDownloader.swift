...
class ImageDownloader: NSObject,NSURLSessionDownloadDelegate, NSURLSessionTaskDelegate, HYLImageFileCache{
    // MARK: - Properties
    ...
    lazy var imageManager = HYLImageManager(directory: NSSearchPathDirectory.CachesDirectory, pathComponents: ["com.HYLMultiImagesViewController.Caches"])
    lazy var imageCache = ImageCache()//cache the image in the memory
    var downloadingURLsList = NSMutableArray()//track the downloading tasks
    ...
    func fileNameForURL(url:NSURL) -> String?{
    ...
    func retrieveImageForURL(url:NSURL) -> UIImage?{
        if let fileName = self.fileNameForURL(url) {
            if let image = self.imageCache.cachedImageForURL(url) {
                return image
            }else if (self.imageManager.imageExistsForImageName(fileName)){
                return self.imageManager.imageWithName(fileName)
            }else{
    ...
    func downloadFileWithURL(url:NSURL){
    ...
    func URLSession(session: NSURLSession, task: NSURLSessionTask, didCompleteWithError error: NSError?) {
        if let completionHandler = self.completionBlock {
            ...
            if let url = task.originalRequest.URL, let fileName = self.fileNameForURL(url) {
                ...
                self.imageCache.cacheImage(image!, withURL: url)
